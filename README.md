...
- ☁️ **官方图片系统** - 云存储 + 云函数架构，支持上传车型封面图
- 👤 **新版登录系统** - 使用微信 `chooseAvatar` 规范接口，头像上传云存储
- 🔐 **游客权限控制** - 游客仅可浏览首页，其他功能需登录
- 📱 **我的页面重构** - 用户资料居中显示，统计数据大数字风格
- 🔒 **上传次数限制** - 每个用户每天最多上传10张图片
- 🔧 **Bug 修复** - 修复头像显示问题，优化登录状态同步
...
- **后端**: 微信云开发 (CloudBase)
- **数据库**: MongoDB (云数据库) - `cars`, `reviews`, `upload_logs` 集合
- **存储**: 微信云存储（车型图片、用户头像）
- **云函数**: 7 个（获取 OpenID、评价管理、分数计算、图片更新等）
